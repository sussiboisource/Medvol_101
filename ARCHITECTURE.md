...
| `New Medvol customers from 1st April 2025.csv` | ✅ exists (108 real rows) | Counter registration log — `Allocated_CounterCode` + `Request_CreatedDate` give each new counter's identity and birth date. Powers Tab 1's New/Old split — with `COUNTER_AGE_CUTOFF_DATE = "auto"` (the default), the cutoff is the earliest date in this file, so every counter listed here becomes "New". Gitignored, same as `brand_master.csv` above. |
